...
- **Raio de presença: 500 m + mapa nas configurações da sala (2026-09-25):**
  - **Padrão 500 m:** migration `20260925000024` — `bars.raio_permitido_metros` default `150` → `500` e `update` dos bars existentes (o `check` 50..1000 continua). A RPC `create_bar` (9 params) é recriada porque `drop function` + `create or replace` não aceita mudar defaults; corpo copiado na íntegra de `20260924000022`. `createBarSchema` e o dialog de criação de bar passam a sugerir 500 m (o campo do cadastro continua editável, 50–1000).
  - **Aviso no toggle:** com `entrada livre` **ligada** o participante entra direto, mas o **gate de presença continua bloqueando** quem estiver fora do raio — a descrição do toggle e o novo card `PresenceGateInfo` (só para o host) deixam isso explícito nos dois estados de `entry_mode`, com o aviso de que o host nunca é bloqueado.
  - **Mapa do raio:** `PresenceRadiusMap` (Leaflet + tiles do **OpenStreetMap**, sem chave de API e sem custo) desenha marcador, círculo do raio em metros, rótulo "500 m a partir do bar" e reenquadra sozinho quando o raio muda — ou seja, acompanha a configuração do host em tempo real. Carregado com `next/dynamic` (`ssr: false`) porque Leaflet não roda no servidor. Links "Abrir no Google Maps"/"Abrir no OpenStreetMap" para o mapa cheio; bar sem coordenadas recebe o aviso de que o gate bloqueia todo participante.
  - **Personalização bloqueada:** o campo "Raio de presença" aparece desabilitado com o valor em vigor (`500 m`) e o rótulo "Personalização em breve" — o host ajusta o raio em fase seguinte.
  - **Por que não o Google Maps Embed (2026-09-25):** o SKU _Embed_ é gratuito e ilimitado, mas exige **API key + billing habilitado** e é um `iframe` **sem círculo/overlay e sem update in-place** — o raio mudaria só recarregando o frame. A porta de entrada para o HUD/sprites animados sobre o mapa está documentada em `src/components/bars/presence-radius-map.tsx`.
  - **Testes:** `src/components/rooms/presence-gate-info.test.tsx` (6) — **`npm test` (180) passa**, com `npm run typecheck`, `npm run lint` e `npm run build`.
